...
describe("processImage", () => {
  ...
  test("returns encoded images", async () => {
    const result = await processImage({ imagePool: imagePoolMock });
  ...
  describe("passes expected options to image.process and image.encode", () => {
    test("when run without options", async () => {
      await processImage({ imagePool: imagePoolMock });
    ...
    test("when run with options promise", async () => {
      await processImage({
        imagePool: imagePoolMock,
        options: () => Promise.resolve(EXPECTED_OPTIONS),
      });
...
describe("getOptions", () => {
  test("returns `undefined` when nothing is passed", async () => {
  ...
  test("passes expected params in function and returns resolved options", async () => {
    const expectedFilePath = "path/to/image.jpg";
    const result = await getOptions({
      filePath: expectedFilePath,
      decodedImage: DECODED_IMAGE,
      options: ({ width, height, size, filePath }) => {
